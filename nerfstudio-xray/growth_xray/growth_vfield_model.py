...
class GrowthVfieldModelConfig(NerfactoModelConfig):
    ...
    growth_activation: Literal['softplus'] = 'softplus'
    """Activation applied to growth_accum. Uses exp(-softplus(g, beta=20))
    which maps growth_accum to a multiplicative factor in (0, 1].
    Fixed to softplus; the config field is retained for documentation."""
    canonical_time: float = 1.0
    """Time of the canonical (reference) frame. 1.0 = last frame."""
    train_velocity: bool = True # when false only freeze velocity, but keep training growth
...
class GrowthVfieldModel(Model):
    ...
    Density query at (x, t):
        ...
        rho(x, t) = rho_canonical × exp(-softplus(growth_accum))
    """
    ...
    def populate_modules(self):
        ...
        # add optional freeze velocity, keep grwoth trainable
        if not self.config.train_velocity:
            for name, param in self.deformation_field.named_parameters():
                if 'growth' not in name:
                    param.requires_grad_(False)
            # 确保 growth 参数可训练（防止被上面的 train_deformation_field=False 误冻结）
            for name, param in self.deformation_field.named_parameters():
                if 'growth' in name:
                    param.requires_grad_(True)
            print(f"[GrowthModel] Velocity FROZEN "
                  f"({sum(1 for n,p in self.deformation_field.named_parameters() if not p.requires_grad)} params frozen), "
                  f"growth trainable "
                  f"({sum(1 for n,p in self.deformation_field.named_parameters() if p.requires_grad)} params active)")
        ...
        # =====================================================================
        # Growth scale: learnable amplification for growth_accum
        # =====================================================================
        # Initialized so that effective_scale = softplus(0.0) + 0.1 = 0.793
        # This gives near-identity behavior at init (close to scale=1.0).
        # The optimizer can push it to scale≈5-10 to achieve strong suppression
        # without needing growth_nn to produce large absolute values.
        #
        # With max_growth≈0.2 and learned scale≈8:
        #   exp(-8 × softplus(0.2)) ≈ exp(-1.6) ≈ 0.20  → 80% suppression
        # Without scale (original, scale=1):
        #   exp(-1 × softplus(0.2)) ≈ exp(-0.2) ≈ 0.82  → only 18% suppression
        self.growth_scale = Parameter(torch.tensor(2.0))
        # effective_scale = softplus(2.0) + 0.1 ≈ 2.33
    ...
    def _apply_growth_activation(self, growth_accum: Tensor) -> Tensor:
        ...
        Scaled model with baseline correction:
            sp(g) = softplus(g, β=20)
            factor = exp(-scale × (sp(g) - sp(0)))

        Subtracting sp(0) = ln(2)/20 ≈ 0.0347 ensures factor = 1.0 EXACTLY
        when growth_accum = 0, regardless of scale. Without this correction,
        scale=8 would cause 24% density loss at canonical time (a bug).

        Scale constraints:
            raw:     softplus(growth_scale, β=1) + 0.1  ∈ (0.1, ∞)
            clamped: clamp(raw, max=20.0)                ∈ [0.1, 20.0]

        Behavior (growth_accum=0.2, scale=8):
            sp(0.2) - sp(0) = 0.200 - 0.035 = 0.165
            factor = exp(-8 × 0.165) = exp(-1.32) = 0.267  → 73% suppression
        """
        raw_scale = torch.nn.functional.softplus(
            self.growth_scale, beta=1.0
        ) + 0.1
        effective_scale = torch.clamp(raw_scale, max=20.0)

        sp = torch.nn.functional.softplus(growth_accum, beta=20.0)
        # Baseline: softplus(0, beta=20) = ln(2)/20
        sp_baseline = 0.034657359027997264  # math.log(2) / 20, constant
        # Clamp ensures factor ≤ 1.0 (no density amplification)
        sp_corrected = torch.clamp(sp - sp_baseline, min=0.0)

        return torch.exp(-effective_scale * sp_corrected)
    ...
    def get_outputs(self, ray_bundle: RayBundle, **kwargs):
        ...
        if 'accum' in growth_storage and growth_storage['accum'] is not None:
            ...
            # exp(-softplus(growth_accum)): returns factor in (0, 1]
    ...
    def get_density_from_pos(
        ...
        rho(x,t) = rho_canonical(warp(x, t->T)) × exp(-softplus(growth_accum))
        """
        if time is None:
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        has_growth = (
            (hasattr(self.deformation_field, 'growth_nn')
             and self.deformation_field.growth_nn is not None)
            or getattr(self.deformation_field, '_use_shared_encoder', False)
        )

        if has_growth:
            use_shared = getattr(
                self.deformation_field, '_use_shared_encoder', False
            )
            if use_shared:
                # Shared encoder: 3-way split
                # shared_backbone → own group (mixed gradients from both heads)
                # vel_head + bspline → velocity group
                # growth_head + growth_bspline → growth group
                backbone_params = []
                velocity_params = []
                growth_params = []
                for n, p in self.deformation_field.named_parameters():
                    if 'shared_backbone' in n:
                        backbone_params.append(p)
                    elif 'growth' in n:
                        growth_params.append(p)
                    else:
                        velocity_params.append(p)

                param_groups["fields"].extend(velocity_params)
                # shared_backbone gets same LR as velocity by default;
                # can be overridden in growth_config.py if needed
                param_groups["fields"].extend(backbone_params)
                param_groups["growth_field"] = growth_params
            else:
                # Independent networks: 2-way split (v3 behavior)
                velocity_params = [
                    p for n, p in self.deformation_field.named_parameters()
                    if 'growth' not in n
                ]
                growth_params = [
                    p for n, p in self.deformation_field.named_parameters()
        ...
        # Add growth_scale to growth_field group so it shares the same
        # optimizer/scheduler as other growth parameters
        if hasattr(self, 'growth_scale'):
            param_groups["growth_field"].append(self.growth_scale)
    ...
    def get_metrics_dict(self, outputs, batch) -> Dict:
        ...
        # Log effective growth_scale for monitoring
        if hasattr(self, 'growth_scale'):
            effective_scale = (
                torch.nn.functional.softplus(self.growth_scale, beta=1.0) + 0.1
            ).item()
            metrics_dict["growth_scale"] = effective_scale
