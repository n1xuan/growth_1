...
class GrowthVfieldModel(Model):
    ...
    def populate_modules(self):
        ...
        # growth_scale removed: exp model + zero-init makes scaling unnecessary
    ...
    def _apply_growth_activation(self, growth_accum: Tensor) -> Tensor:
        ...
        Multiplicative model: density = canonical × exp(-softplus(growth_accum))
    
        Using softplus(x, beta=20) ≈ ReLU(x) but C∞ smooth:
        - growth_accum = 0  → softplus(0) ≈ 0.035 → factor ≈ 0.966
        - growth_accum = 1  → softplus(1) ≈ 1.0   → factor ≈ 0.37
        - growth_accum = 3  → softplus(3) ≈ 3.0   → factor ≈ 0.05
        - growth_accum < 0  → softplus → 0         → factor → 1.0
        - factor is ALWAYS > 0 → density is always non-negative
        - gradient is ALWAYS nonzero → no dead neurons
    
        Physical interpretation: growth_accum is the time-integrated growth rate
        from query time t to canonical time T. Positive values indicate material
        that was absent at time t but present at time T (new solid phase).
        """
