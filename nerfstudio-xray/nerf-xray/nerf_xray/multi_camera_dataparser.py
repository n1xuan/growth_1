...
@dataclass
class MultiCameraDataParserConfig(DataParserConfig):
    """Multi-camera dataset config"""

    _target: Type = field(default_factory=lambda: MultiCameraDataParser)
    ...
    eval_mode: Literal["fraction", "filename", "filename+modulo", "interval", "all"] = "filename+modulo"
    """
    ...
    """Whether to use camera weights for multi-camera images."""
...
class MultiCameraDataParser(Nerfstudio):
    """Multi-camera DataParser that supports multiple cameras per image"""
    config: MultiCameraDataParserConfig
    downscale_factors: Dict[str, float] = {}

    def __init__(self, config: MultiCameraDataParserConfig):
        super().__init__(config)
        self.includes_time = config.includes_time
        self.use_camera_weights = config.use_camera_weights
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        depth_filenames = []
        poses = []
        camera_indices = []  # for each frame store a list of camera indices
        camera_weights = []  # Store weights for each camera
        ...
        for i_frame, frame in enumerate(frames):
            ...
            # Add the same image filename for each camera
            ...
            # Handle multiple cameras per frame based on transform_matrix shape
            transform_matrices = np.array(frame["transform_matrix"])
            if transform_matrices.shape == (4, 4):
                # Single camera case
                transform_matrices = transform_matrices[np.newaxis, ...]  # Add batch dimension
                num_cameras = 1
            else:
                # Multi-camera case
                num_cameras = transform_matrices.shape[0]
            
            _times = frame.get("time", [0.0])
            if isinstance(_times, float) or isinstance(_times, int):
                _times = [_times]
            
            # Ensure we have the same number of transforms and times
            assert len(_times) == num_cameras, "Number of times and number of cameras do not match"

            camera_indices.append([k + len(poses) for k in range(num_cameras)])
            
            # Process each camera in the frame
            for i in range(num_cameras):
                # selection based on timestamp
                if _times[i] < self.config.min_timestamp or _times[i] > self.config.max_timestamp:
                    continue
                
                # Add camera data to lists
                poses.append(transform_matrices[i])
                times.append(float(_times[i]))
                if 'camera_weights' in frame:
                    if isinstance(frame['camera_weights'], list):
                        camera_weights.append(frame['camera_weights'][i])
                    else:
                        camera_weights.append(frame['camera_weights'])
                else:
                    camera_weights.append(1.0)
        ...
        # need to see which cameras belong to the selected images
        chosen_cameras = [camera_indices[i] for i in indices] # [num_frames, num_cameras_per_frame]
        idx_tensor = torch.tensor(chosen_cameras, dtype=torch.long).flatten()
        idx_list = idx_tensor.tolist()
        poses = poses[idx_tensor]
        # need new camera indices; camera indices are mapping which assigns a list of cameras to each frame
        camera_indices = [camera_indices[i] for i in indices]
        for i_row, camera_nums in enumerate(camera_indices):
            camera_indices[i_row] = [idx_list.index(i) for i in camera_nums]
        camera_indices = np.array(camera_indices)
        camera_indices = torch.tensor(camera_indices, dtype=torch.long)
        # image_indices are the inverse mapping - i.e. assign a frame index to each camera
        image_indices = [[i_row]*len(camera_nums) for i_row, camera_nums in enumerate(camera_indices)]
        image_indices = torch.tensor(image_indices, dtype=torch.long).flatten().reshape(-1, 1)
        ...
        camera_weights = torch.tensor(camera_weights, dtype=torch.float32)[idx_tensor].reshape(-1, 1)
        if distort_fixed:
            distortion_params = (
        ...
        # Add camera weights to metadata if enabled
        if self.use_camera_weights:
            metadata["camera_weights"] = camera_weights

        # Add image indices to metadata to track which cameras belong to which images
        metadata["image_indices"] = image_indices
        ...
        return DataparserOutputs(
            ...
            metadata={
                ...
                "image_timestamps": times.reshape(camera_indices.shape).mean(dim=1).tolist(),
                **metadata,
            },
        ) 